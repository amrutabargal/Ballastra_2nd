...
export default function NotificationSettingsScreen({ navigation, route }) {
  ...
  // Notification control states
  const [everyMessage, setEveryMessage] = useState(true);
  const [mentionsOnly, setMentionsOnly] = useState(false);
  const [never, setNever] = useState(false);

  const spaceName = route?.params?.spaceName || "general";

  const getSelectedLabel = () => {
    const option = DURATION_OPTIONS.find((opt) => opt.value === selectedDuration);
    return option ? option.label : "15 Minute";
  };
  ...
  // Handle toggle to ensure only one option is active at a time
  const handleEveryMessage = (value) => {
    if (value) {
      setEveryMessage(true);
      setMentionsOnly(false);
      setNever(false);
    } else {
      setEveryMessage(false);
    }
  };

  const handleMentionsOnly = (value) => {
    if (value) {
      setEveryMessage(false);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTextContainer}>
          ...
          <View style={styles.channelNameRow}>
            ...
            <Text style={styles.channelName}>{spaceName}</Text>
          </View>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        ...
        <View style={styles.durationValueContainer}>
          ...
          <Ionicons name="chevron-expand-outline" size={16} color="#9AA4C7" />
        </View>
      </TouchableOpacity>
      ...
      <Text style={styles.descriptionText}>
        Muted space won't send you notifications and will appear dimmed in your space list. This setting syncs across all your devices.
      </Text>
