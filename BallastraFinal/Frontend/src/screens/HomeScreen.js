...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    const fetchNexus = async () => {
      try {
        ...
        let allNexus = [];
        const userOwnNexusIds = new Set();

        // If user is logged in, fetch their own nexus (both public and private)
        if (token) {
          try {
            const myRes = await fetch(`${BASE_URL}/api/nexus/my`, {
              method: "GET",
              headers: {
            ...
            if (myRes.ok) {
              ...
              if (myJson.success && Array.isArray(myJson.data)) {
                // Add user's own nexus to the list
                myJson.data.forEach((item) => {
                  allNexus.push(item);
                  userOwnNexusIds.add(item.id.toString());
                });
              }
            }
          } catch (err) {
            console.log("Error fetching user's nexus:", err.message);
          }
        }

        // Fetch all public nexus
        try {
          const publicRes = await fetch(`${BASE_URL}/api/nexus/public`, {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          if (publicRes.ok) {
            ...
            if (publicJson.success && Array.isArray(publicJson.data)) {
              // Add public nexus, but exclude user's own to avoid duplicates
              publicJson.data.forEach((item) => {
                const itemId = (item.id || item._id || "").toString();
                if (!userOwnNexusIds.has(itemId)) {
                  allNexus.push(item);
                }
              });
            }
          ...
          console.log("Error fetching public nexus:", err.message);
        }

        // Map the combined nexus list to the expected format
        if (allNexus.length > 0) {
          const token = await AsyncStorage.getItem("token");
          const userData = token ? JSON.parse(await AsyncStorage.getItem("user") || "{}") : null;
          const currentUserId = userData?.id || userData?._id || null;

          const mapped = allNexus.map((item, index) => ({
            id:
              (item._id && item._id.toString()) ||
              (item.id && item.id.toString()) ||
              String(index + 1),
            ...
            members: item.members || item.membersCount || 0,
            type: item.type || "Community",
            description: item.description || "Night grind + neon vibes",
            description2: item.description2 || item.shortDescription || "Chill gamers. Cozy lobbies",
            image:
              item.image ||
              item.imageUrl ||
              item.icon ||
              "https://images.pexels.com/photos/337909/pexels-photo-337909.jpeg?auto=compress&cs=tinysrgb&w=800",
            // Store additional metadata for invite/join functionality
            is_public: item.is_public !== false, // Default to true if not specified
            created_by: item.created_by || item.createdBy || null,
            isOwner: currentUserId && (item.created_by?.toString() === currentUserId?.toString() || item.createdBy?.toString() === currentUserId?.toString()),
          }));

          setCommunities(mapped);
        }
        // If no data, keep using fallback COMMUNITIES
      } catch (err) {
        // Silently fail and use fallback data - don't show alert
        console.log("Network error fetching nexus, using fallback data:", err.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleInviteOrJoin = async (item, event) => {
    ...
    try {
      ...
      if (!token) {
        Alert.alert("Login Required", "Please login to join or invite users to nexus");
        return;
      }

      // Check if nexus is public or private
      const isPublic = item.is_public !== false;
      const isOwner = item.isOwner;

      if (isPublic) {
        // Public nexus: Any user can join directly
        try {
          const response = await fetch(`${BASE_URL}/api/members/${item.id}`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
            ...
            body: JSON.stringify({
              userId: null, // Will use current user from token
              role: "member",
            }),
          });
          ...
          if (response.ok && data?.success) {
            Alert.alert("Success", `You joined ${item.name}!`);
            // Update joined state
            setJoined((prev) => new Set([...prev, item.id]));
            // Refresh the list
            const fetchNexus = async () => {
              // Re-fetch to update member count
              // This is a simple approach - you might want to optimize this
            };
          } else {
            if (data?.message?.includes("Already") || data?.message?.includes("already")) {
              Alert.alert("Already Joined", `You are already a member of ${item.name}`);
            } else {
              Alert.alert("Error", data?.message || "Failed to join nexus");
            }
          }
        } catch (error) {
          console.error("Join nexus error:", error);
          Alert.alert("Error", "Failed to join nexus. Please try again.");
        }
      } else {
        // Private nexus: Only owner/admin can generate invite link
        if (isOwner) {
          try {
            const response = await fetch(`${BASE_URL}/api/invite/generate`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
            ...
            if (response.ok && data?.success && data?.data?.link) {
              // Copy link to clipboard or show share options
              Alert.alert(
                "Invite Link Generated",
                `Invite link: ${data.data.link}\n\nThis link will expire in 7 days.`,
                [
                  { text: "OK" },
                  {
                    text: "Share",
                    onPress: () => {
                      // Navigate to share screen or use sharing API
                      navigation.navigate("Share_Profile", {
                        inviteLink: data.data.link,
                        nexusName: item.name,
                      });
                    },
                  },
              ...
              Alert.alert("Error", data?.message || "Failed to generate invite link");
            }
          } catch (error) {
          ...
          Alert.alert(
            "Private Nexus",
            "This is a private nexus. Only the creator can invite members. Please request an invite link from the nexus owner."
          );
        }
      }
    } catch (error) {
          ...
          <View style={styles.cardContent}>
            <View style={styles.cardHeader}>
              ...
              <TouchableOpacity 
                ...
                onPress={(e) => handleInviteOrJoin(item, e)}
                activeOpacity={0.7}
              >
                <Icon
                  name={isJoined ? "check" : item.is_public ? "users" : "link"}
                  size={16}
                  color="#fff"
                />
              </TouchableOpacity>
